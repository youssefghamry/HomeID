...
$layout_matrix = G5ERE()->listing()->get_layout_matrix($post_layout);
...
$image_ratio = '';
if ($image_size === 'full') {
	$image_ratio_custom = isset($post_settings['image_ratio']) ? $post_settings['image_ratio'] : G5ERE()->options()->get_option('post_image_ratio');
...
$wrapper_classes = array(
	'g5ere__listing-wrap',
	"g5ere__property_layout-{$post_layout}"
);
...
<?php if (G5CORE()->query()->have_posts()): ?>
	<div <?php echo join(' ', $wrapper_attributes); ?> class="<?php echo esc_attr($wrapper_class) ?>">
		...
		do_action('g5core_before_listing_wrapper', $post_settings);
		?>
				...
				if (is_array($layout_settings)) {
					...
					while (G5CORE()->query()->have_posts()) {
						...
						$current_post_classes = array(
							$template_class,
							"g5ere__property-item-{$post_index}"
						);

						$property_featured = get_post_meta(get_the_ID(),G5ERE()->ere_meta_prefix. 'property_featured',true);
						if ($property_featured === '1') {
							$current_post_classes[] = 'g5ere__property-item-featured';
						}
						...
						do_action('g5ere_before_get_template_listing_property_item',$template);
						...
						do_action('g5ere_after_get_template_listing_property_item',$template);
					...
					G5ERE()->get_template( "loop/listing/{$post_layout}.php",array(
		...
		<?php
		// You can use this for adding codes before the main loop
		do_action('g5core_after_listing_wrapper',$post_settings);
		?>
	</div>
<?php elseif (isset($post_settings['isMainQuery'])): ?>
	<?php G5ERE()->get_template( 'loop/content-none.php' ); ?>
<?php endif; ?>
