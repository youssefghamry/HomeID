 ...
 * @var $css_class_field
 */
$current_min_area = isset($_REQUEST['min-land-area']) ? ere_clean(wp_unslash($_REQUEST['min-land-area'] )) : '';
$current_max_area = isset($_REQUEST['max-land-area']) ? ere_clean(wp_unslash($_REQUEST['max-land-area'] )) : '';
$min_area = ere_get_option('property_land_slider_min', 0);
$max_area = ere_get_option('property_land_slider_max', 1000);
$measurement_units = ere_get_measurement_units_land_area();
...
<div class="<?php echo esc_attr($wrapper_class)?>">
	<div class="g5ere__range-slider-wrap" data-toggle="g5ere__range-slider" data-options="<?php echo esc_attr(json_encode($range_slider_options))?>">
		<div class="g5ere__rs-text">
			<input type="hidden" name="min-land-area" class="g5ere__rsi-min g5ere__rs-input" value="<?php echo esc_attr($current_min_area === $min_area ? '' : $current_min_area)?>">
			<input type="hidden" name="max-land-area" class="g5ere__rsi-max g5ere__rs-input" value="<?php echo esc_attr($current_max_area === $max_area ? '' : $current_max_area)?>">
			<span class="g5ere__rs-title"><?php echo esc_html__('Land Area Range:','g5-ere'); ?></span> <?php echo esc_html__('from','g5-ere'); ?> <span class="g5ere__rst-min"><?php echo ere_get_format_number($current_min_area) ?></span> <span class="g5ere__rst-prefix"><?php echo wp_kses_post($measurement_units)?></span> <?php echo esc_html__('to','g5-ere'); ?> <span class="g5ere__rst-max"><?php echo ere_get_format_number($current_max_area) ?></span> <span class="g5ere__rst-prefix"><?php echo wp_kses_post($measurement_units)?></span>
		</div>
