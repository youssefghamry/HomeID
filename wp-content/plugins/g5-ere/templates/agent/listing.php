...
$post_settings = &G5ERE()->listing_agent()->get_layout_settings();
...
$layout_matrix     = G5ERE()->listing_agent()->get_layout_matrix( $post_layout );
...
$image_ratio = '';
if ( $image_size === 'full' ) {
	$image_ratio_custom = isset( $post_settings['image_ratio'] ) ? $post_settings['image_ratio'] : G5ERE()->options()->get_option( 'agent_image_ratio' );
...
$wrapper_classes = array(
	'g5ere__listing-wrap',
	"g5ere__listing-agent-layout-{$post_layout}"
);
        ...
        <div <?php echo join( ' ', $inner_attributes ); ?> class="<?php echo esc_attr( $inner_class ); ?>">
			...
			if ( is_array( $layout_settings ) ) {
				...
				while ( G5CORE()->query()->have_posts() ) {
					...
					do_action( 'g5ere_before_get_template_listing_agent_item', $template );
					...
					do_action( 'g5ere_after_get_template_listing_agent_item', $template );
				...
				G5ERE()->get_template( "agent/loop/listing/{$post_layout}.php", array(
...
<?php elseif ( isset( $post_settings['isMainQuery'] ) ): ?>
	<?php G5ERE()->get_template( 'agent/loop/content-none.php' ); ?>
<?php endif; ?>
