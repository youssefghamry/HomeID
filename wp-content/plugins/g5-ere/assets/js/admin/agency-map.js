...
(function ($) {
	G5ERE_ADMIN_MAP = {
		...
		updateMap: function () {
			var self = this;
			if (self.map) {
				var $address = $('.g5ere__agency_address'),
					$lockPin = $('.g5ere__map-lock-pin'),
					$currentLocation = $('.g5ere__current-location');
				this.lat = self.map.options.position.lat ? self.map.options.position.lat : g5ere_admin_agency_map_vars.coordinate_default.lat;
				this.lng = self.map.options.position.lng ? self.map.options.position.lng : g5ere_admin_agency_map_vars.coordinate_default.lng;
				...
				$('[name="real_estate_map_lat"],[name="real_estate_map_lng"]').on('change', function () {
					...
					if (latLng) {
						...
						marker.setPosition(latLng);
						self.map.resetZoom();
						$('[name="real_estate_agency_location[location]"]').val(self.lat + ',' + self.lng);
					} else {
						self.setPosition();
					}
				});
		...
		setPosition: function () {
			...
			$('[name="real_estate_agency_location[location]"]').val(this.lat + ',' + this.lng);
		},
