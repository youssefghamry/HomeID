...
<div class="<?php echo esc_attr( $settings['column_class'] ) ?>">
    <article class="card ube-post-item ube-post-grid-layout-04">
		<?php
		if ( has_post_thumbnail() && $settings['show_image'] == 'yes' ) :
            ...
            <a class="ube-entry-post-thumb card-img-top" href="<?php echo esc_url( get_the_permalink() ) ?>"
               style="background-image: url('<?php echo esc_url( wp_get_attachment_image_url( get_post_thumbnail_id(), 'full' ) ) ?>');<?php if ( isset( $settings['ratio'] ) ) { ?>--ube-post-grid-ratio:<?php echo $settings['ratio'] ?>;<?php } ?>">
            </a>
		<?php endif; ?>
        <div class="card-body">
