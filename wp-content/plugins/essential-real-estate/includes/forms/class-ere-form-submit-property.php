...
if ( ! class_exists( 'ERE_Form_Submit_Property' ) ) {
	...
	class ERE_Form_Submit_Property extends ERE_Form {
		...
		public function __construct() {
			...
			if ( isset( $_POST['step'] ) ) {
				$this->step = is_numeric( ere_clean( wp_unslash( $_POST['step'] ) ) ) ? max( absint( ere_clean( wp_unslash( $_POST['step'] ) ) ), 0 ) : array_search( ere_clean( wp_unslash( $_POST['step'] ) ), array_keys( $this->steps ) );
			} elseif ( ! empty( $_GET['step'] ) ) {
				$this->step = is_numeric( ere_clean( wp_unslash( $_GET['step'] ) ) ) ? max( absint( ere_clean( wp_unslash( $_GET['step'] ) ) ), 0 ) : array_search( ere_clean( wp_unslash( $_GET['step'] ) ), array_keys( $this->steps ) );
			}

			$this->property_id = ! empty( $_REQUEST['property_id'] ) ? absint( ere_clean( wp_unslash( $_REQUEST['property_id'] ) ) ) : 0;
		...
		public function submit_handler() {
			...
			try {
				...
				echo '<div class="ere-error">' . esc_html($e->getMessage())  . '</div>';
