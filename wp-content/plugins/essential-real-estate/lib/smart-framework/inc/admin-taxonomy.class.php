	...
	class GSF_Inc_Admin_Taxonomy
	{
		...
		public function ajaxMetaForm() {
		    if (isset($_REQUEST['taxonomy'])) {
			    $taxonomy = GSF()->helper()->sanitize_text($_REQUEST['taxonomy']);
		...
		public function updateTermMeta($term_id, $tag_id) {
            ...
            $current_taxonomy = GSF()->helper()->sanitize_text($_POST['taxonomy']);
		...
		public function saveTermMeta($term_id, $tag_id) {
			...
			foreach ($meta_field_keys as $meta_id => $field_meta) {
				$meta_value = isset($_POST[$meta_id])
                    ? GSF()->helper()->sanitize_text($_POST[$meta_id])
                    : $field_meta['empty_value'];
				update_term_meta($term_id, $meta_id, $meta_value);
			}
		}
		...
		private function getMetaValue(&$configs) {
			$id = isset($_GET['tag_ID']) ? GSF()->helper()->sanitize_text($_GET['tag_ID']) : '0';
			$meta_values = array();
