...
if ( ! class_exists( 'ERE_Admin_Property' ) ) {
	...
	class ERE_Admin_Property {
		...
		public function register_custom_column_titles( $columns ) {
			...
			$columns['featured']  = '<span data-tip="' . esc_attr__( 'Featured?', 'essential-real-estate' ) . '" class="tips dashicons dashicons-star-filled"></span>';
		...
		public function display_custom_column( $column ) {
			...
			switch ( $column ) {
				...
				case 'featured':
					...
					if ( $featured == 1 ) {
						$featured_text = esc_html__( 'Featured', 'essential-real-estate' );
						$featured_icon = 'tips accent-color dashicons dashicons-star-filled';
					} else {
						$featured_text = esc_html__( 'Not Feature', 'essential-real-estate' );
						$featured_icon = 'tips dashicons dashicons-star-empty';
					}

					$url = wp_nonce_url( admin_url( 'admin-ajax.php?action=ere_admin_feature_property&id=' . $post->ID ), 'dip-feature-property' );
				...
				case 'viewcount' :
					...
					echo esc_html( ere_get_format_number( $views ) );
					break;
			}
		}
		...
		public function modify_list_row_actions( $actions, $post ) {
			...
			if ( $post->post_type == 'property' ) {
				if ( in_array( $post->post_status, array(
						'pending',
						'expired'
					) ) && current_user_can( 'publish_propertys', $post->ID ) ) {
		...
		public function approve_property() {
			if ( ! empty( $_GET['approve_listing'] ) && wp_verify_nonce( ere_clean( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'approve_listing' ) && current_user_can( 'publish_post', $_GET['approve_listing'] ) ) {
				$post_id      = absint( ere_clean( wp_unslash( $_GET['approve_listing'] ) ) );
				$listing_data = array(
					'ID'          => $post_id,
					'post_status' => 'publish'
				);
		...
		public function expire_property() {
			if ( ! empty( $_GET['expire_listing'] ) && wp_verify_nonce( ere_clean( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'expire_listing' ) && current_user_can( 'publish_post', $_GET['expire_listing'] ) ) {
				$post_id      = absint( ere_clean( wp_unslash( $_GET['expire_listing'] ) ) );
				$listing_data = array(
					'ID'          => $post_id,
					'post_status' => 'expired'
				);
		...
		public function hidden_property() {
			if ( ! empty( $_GET['hidden_listing'] ) && wp_verify_nonce( ere_clean( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'hidden_listing' ) && current_user_can( 'publish_post', $_GET['hidden_listing'] ) ) {
				$post_id      = absint( ere_clean( wp_unslash( $_GET['hidden_listing'] ) ) );
				$listing_data = array(
					'ID'          => $post_id,
					'post_status' => 'hidden'
				);
		...
		public function show_property() {
			if ( ! empty( $_GET['show_listing'] ) && wp_verify_nonce( ere_clean( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'show_listing' ) && current_user_can( 'publish_post', $_GET['show_listing'] ) ) {
				$post_id      = absint( ere_clean( wp_unslash( $_GET['show_listing'] ) ) );
				$listing_data = array(
					'ID'          => $post_id,
					'post_status' => 'publish'
				);
		...
		public function filter_restrict_manage_property() {
			...
			if ( $typenow == $post_type ) {
				...
				?>
				<input type="text" placeholder="<?php esc_attr_e( 'Author', 'essential-real-estate' ); ?>"
				       name="property_author" value="<?php echo esc_attr( $property_author ); ?>">
				<input type="text" placeholder="<?php esc_attr_e( 'Property ID', 'essential-real-estate' ); ?>"
				       name="property_identity" value="<?php echo esc_attr( $property_identity ); ?>">
				<?php
			};
		}
		...
		public function feature() {
			...
			if ( ! wp_verify_nonce( ere_clean( wp_unslash( $_GET['_wpnonce'] ) ), 'dip-feature-property' ) ) {
				return;
			}
			...
			update_post_meta( $property_id, ERE_METABOX_PREFIX . 'property_featured', $is_featured );
			if ( $is_featured == 1 ) {
			...
			wp_safe_redirect( wp_get_referer() ? remove_query_arg( array(
				...
				'deleted',
				'ids'
			), wp_get_referer() ) : admin_url( 'edit.php?post_type=property' ) );
			die();
		}
	}
}