    ...
    $(document).ready(function () {
        if (typeof ere_property_vars !== "undefined") {
            ...
            var ajax_url = ere_property_vars.ajax_url,
                ajax_upload_url = ere_property_vars.ajax_upload_url,
                ajax_upload_attachment_url = ere_property_vars.ajax_upload_attachment_url,
                ...
                googlemap_coordinate_default = ere_property_vars.googlemap_coordinate_default,
            ...
            var ere_property_gallery_event = function () {
                ...
                // Set Featured Image
                $('.icon-featured', '.ere-property-gallery').off('click').on('click', function () {
                ...
                $('.icon-delete', '.ere-property-gallery').off('click').on('click', function () {
                    var $this = $(this),
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.success) {
                                ...
                                if ($('.ere-property-gallery').find('[name="featured_image_id"]').length === 0) {
                                    var $firstImage = $('.ere-property-gallery').find('.media-thumb-wrap');
                                    if ($firstImage.length > 0) {
                                        $firstImage = $($firstImage[0]);
                                        var attachment_id  = $firstImage.find('.property_image_ids').val();
            ...
            var ere_property_attachments_event = function () {
                $('.icon-delete', '.ere-property-attachments').off('click').on('click', function () {
                    var $this = $(this),
	                    $wrap = $this.closest('.media-thumb-wrap'),
	                    file_id = $wrap.attr('id'),
                        icon_delete = $this.children('i'),
            ...
            var ere_property_gallery_images = function () {
	            ...
	            uploader_gallery.bind('FilesAdded', function (up, files) {
                    var propertyThumb = "";
                    var maxfiles = max_property_images;
                    var totalFiles = $('#property_gallery_thumbs_container').find('.__thumb').length + up.files.length;
            ...
            var ere_property_attachments = function () {
	            ...
	            uploader_attachments.bind('FilesAdded', function (up, files) {
		            ...
		            var totalFiles = $('#property_attachments_thumbs_container').find('.__thumb').length + up.files.length;
                    if (totalFiles > maxfiles) {
	                    $.each(files, function(i, file) {
		                    up.removeFile(file);
	                    });
            ...
            $('#property_type,#property_status,#property_label',css_class_wrap).select2();
